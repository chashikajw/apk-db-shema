...
CREATE SEQUENCE RUNTIME_SEQUENCE START WITH 1 INCREMENT BY 1;
CREATE TABLE IF NOT EXISTS RUNTIME_ENV (
    RUNTIME_ID INTEGER DEFAULT nextval('runtime_sequence'),
    RUNTIME_UUID VARCHAR(256),
    RUNTIME_NAME VARCHAR(256),
    ORGANIZATION VARCHAR(100),
    CREATED_BY VARCHAR(100),
    CREATED_TIME TIMESTAMP,
    UPDATED_BY VARCHAR(100),
...
CREATE SEQUENCE RUNTIME_API_SEQUENCE START WITH 1 INCREMENT BY 1;
CREATE TABLE IF NOT EXISTS RUNTIME_API (
    RUNTIME_API_ID INTEGER DEFAULT nextval('runtime_api_sequence'),
    RUNTIME_API_UUID VARCHAR(256),
    RUNTIME_ID VARCHAR(256),
    RUNTIME_API_PROVIDER VARCHAR(256),
    RUNTIME_API_NAME VARCHAR(256),
    RUNTIME_API_VERSION VARCHAR(30),
    CONTEXT VARCHAR(256),
    CONTEXT_TEMPLATE VARCHAR(256),
    RUNTIME_API_TIER VARCHAR(256),
    RUNTIME_API_TYPE VARCHAR(10),
    ORGANIZATION VARCHAR(100),
    ...
    PRIMARY KEY(RUNTIME_API_ID),
    FOREIGN KEY (RUNTIME_ID) REFERENCES RUNTIME_ENV(RUNTIME_UUID) ON UPDATE CASCADE ON DELETE CASCADE,
    UNIQUE (RUNTIME_API_PROVIDER,RUNTIME_API_NAME,RUNTIME_API_VERSION,ORGANIZATION),
    UNIQUE (RUNTIME_API_UUID)
);
...
CREATE SEQUENCE RUNTIME_API_AM_API_MAPPING_SEQUENCE START WITH 1 INCREMENT BY 1;
CREATE TABLE IF NOT EXISTS RUNTIME_API_AM_API_MAPPING (
	AM_API_MAPPING_ID INTEGER DEFAULT nextval('runtime_api_am_api_mapping_sequence'),
	RUNTIME_API_ID INTEGER,
	API_ID INTEGER,
	REVISION_UUID VARCHAR(255),
	FOREIGN KEY (RUNTIME_API_ID) REFERENCES RUNTIME_API(RUNTIME_API_ID) ON DELETE CASCADE,
	FOREIGN KEY (API_ID) REFERENCES AM_API(API_ID) ON DELETE CASCADE,
	PRIMARY KEY(AM_API_MAPPING_ID)
);
...
CREATE TABLE IF NOT EXISTS AM_GRAPHQL_COMPLEXITY (
    UUID VARCHAR(256),
    RUNTIME_API_ID INTEGER NOT NULL,
    TYPE VARCHAR(256),
    FIELD VARCHAR(256),
    COMPLEXITY_VALUE INTEGER,
    REVISION_UUID VARCHAR(255),
    FOREIGN KEY (RUNTIME_API_ID) REFERENCES RUNTIME_API(RUNTIME_API_ID) ON UPDATE CASCADE ON DELETE CASCADE,
    PRIMARY KEY(UUID)
);

CREATE SEQUENCE RUNTIME_API_URL_MAPPING_SEQUENCE START WITH 1 INCREMENT BY 1;
CREATE TABLE IF NOT EXISTS RUNTIME_API_URL_MAPPING (
    URL_MAPPING_ID INTEGER DEFAULT nextval('runtime_api_url_mapping_sequence'),
    RUNTIME_API_ID INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS RUNTIME_API_RESOURCE_SCOPE_MAPPING (
    SCOPE_NAME VARCHAR(256) NOT NULL,
    URL_MAPPING_ID INTEGER NOT NULL,
    TENANT_ID INTEGER NOT NULL,
    FOREIGN KEY (URL_MAPPING_ID) REFERENCES RUNTIME_API_URL_MAPPING(URL_MAPPING_ID) ON DELETE CASCADE,
...
CREATE SEQUENCE RUNTIME_API_THROTTLE_POLICY_SEQ START WITH 1 INCREMENT BY 1;
CREATE TABLE IF NOT EXISTS RUNTIME_API_THROTTLE_POLICY (
            POLICY_ID INTEGER DEFAULT NEXTVAL('runtime_api_throttle_policy_seq'),
            NAME VARCHAR(512) NOT NULL,
            DISPLAY_NAME VARCHAR(512) NULL DEFAULT NULL,
            ORGANIZATION VARCHAR(100) NOT NULL,
            DESCRIPTION VARCHAR (1024),
...
CREATE TABLE IF NOT EXISTS AM_CONDITION_GROUP (
            ...
            FOREIGN KEY (POLICY_ID) REFERENCES RUNTIME_API_THROTTLE_POLICY(POLICY_ID) ON DELETE CASCADE ON UPDATE CASCADE
);
...
DROP TABLE IF EXISTS RUNTIME_API_CLIENT_CERTIFICATE;
CREATE TABLE RUNTIME_API_CLIENT_CERTIFICATE (
  TENANT_ID INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS AM_REVISION (
            ID INTEGER NOT NULL,
            RUNTIME_API_UUID VARCHAR(256) NOT NULL,
            REVISION_UUID VARCHAR(255) NOT NULL,
            DESCRIPTION VARCHAR(255),
            CREATED_TIME TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
            CREATED_BY VARCHAR(255),
            PRIMARY KEY (ID, RUNTIME_API_UUID),
            FOREIGN KEY (RUNTIME_API_UUID) REFERENCES RUNTIME_API(RUNTIME_API_UUID) ON DELETE CASCADE,
            UNIQUE(REVISION_UUID)
);

DROP TABLE IF EXISTS RUNTIME_API_REVISION_METADATA;
CREATE TABLE IF NOT EXISTS RUNTIME_API_REVISION_METADATA (
    RUNTIME_API_UUID VARCHAR(64),
    REVISION_UUID VARCHAR(255),
    API_TIER VARCHAR(128),
    ARTIFACT JSONB NOT NULL,
    FOREIGN KEY(REVISION_UUID) REFERENCES AM_REVISION(REVISION_UUID) ON DELETE CASCADE,
    UNIQUE (RUNTIME_API_UUID,REVISION_UUID)
);
...
CREATE TABLE IF NOT EXISTS RUNTIME_API_ENVIRONMENT_KEYS
(
    UUID            VARCHAR(45)  NOT NULL,
    ENVIRONMENT_ID  VARCHAR(45)  NOT NULL,
    RUNTIME_API_UUID        VARCHAR(256) NOT NULL,
    PROPERTY_CONFIG bytea DEFAULT NULL,
    UNIQUE (ENVIRONMENT_ID, RUNTIME_API_UUID),
    FOREIGN KEY (RUNTIME_API_UUID) REFERENCES RUNTIME_API (RUNTIME_API_UUID) ON DELETE CASCADE,
    PRIMARY KEY (UUID)
);
...
DROP TABLE IF EXISTS RUNTIME_API_OPERATION_POLICY;
CREATE TABLE IF NOT EXISTS RUNTIME_API_OPERATION_POLICY (
   API_SPECIFIC_POLICY_ID SERIAL,
   POLICY_UUID VARCHAR(45) NOT NULL,
   RUNTIME_API_UUID VARCHAR(45) NOT NULL,
...
DROP TABLE IF EXISTS RUNTIME_API_OPERATION_POLICY_MAPPING;
CREATE TABLE IF NOT EXISTS RUNTIME_API_OPERATION_POLICY_MAPPING (
...
create index IDX_AAUM_AI on RUNTIME_API_URL_MAPPING (RUNTIME_API_ID);
create index IDX_AAUM_TT on RUNTIME_API_URL_MAPPING (THROTTLING_TIER);
create index IDX_AATP_DQT on RUNTIME_API_THROTTLE_POLICY (DEFAULT_QUOTA_TYPE);
